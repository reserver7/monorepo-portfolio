...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn("bg-surface fixed z-50 p-6 shadow-lg transition ease-in-out", sideClass[side], className)}
      {...props}
    >
      {children}
      <DialogPrimitive.Close className="text-muted focus:ring-primary absolute right-4 top-4 rounded-sm opacity-80 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2">
        <X className="h-4 w-4" />
