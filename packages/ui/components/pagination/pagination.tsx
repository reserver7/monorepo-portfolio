...
const PaginationBase = React.forwardRef(function PaginationBase(
  ...
  const normalizedPageSizeOptions = React.useMemo(() => {
    if (Array.isArray(pageSizeOptions)) {
      return Array.from(new Set(pageSizeOptions.map((value) => toSafePageSize(Number(value))))).sort(
        (a, b) => a - b
      );
    }
    if (typeof pageSizeOptions === "string") {
      try {
        ...
        if (Array.isArray(parsed)) {
          return Array.from(new Set(parsed.map((value) => toSafePageSize(Number(value))))).sort(
            (a, b) => a - b
          );
        }
      } catch {
  ...
  const stableCellStyle = React.useMemo(
    () => ({ width: pageCellWidth, ...stableNumericStyle }),
    [pageCellWidth, stableNumericStyle]
  );
    ...
    ({
      ...
      isMinimalItemStyle ? (
        <button
          ...
          onClick={onClick}
          className={cn(
            "text-muted inline-flex items-center justify-center transition-colors",
      ...
      isMinimalItemStyle ? (
        <button
          ...
          className={cn(
            ...
            paginationCellClass,
            value === currentPage
              ? "ring-primary/35 bg-primary/12 text-primary font-semibold ring-1"
    ...
    >
      <div
        className={cn(
          ...
          fullWidth ? "w-full flex-wrap justify-center" : "flex-wrap"
        )}
        style={stableNumericStyle}
      >
        {showFirstLast
          ? renderIconControl({
              ariaLabel: "첫 페이지",
        ...
        {simple ? (
          <span
            className={cn(
              "border-default bg-surface text-foreground inline-flex items-center justify-center rounded-[var(--radius-md)] border px-[var(--space-3)] font-medium",
      ...
      {hasTools ? (
        <div
          className={cn(
            "text-body-sm text-muted flex flex-wrap items-center gap-[var(--space-2)]",
        ...
        >
          ...
          {showPageSizeSelector ? (
            <label className="inline-flex items-center gap-[var(--space-1)]">
              ...
              <select
                className="border-default bg-surface text-body-sm text-foreground h-[var(--size-control-sm)] rounded-[var(--radius-md)] border px-[var(--space-2)]"
          ...
          {showQuickJumper ? (
            <div className="inline-flex items-center gap-[var(--space-1)]">
              <input
                className="border-default bg-surface text-body-sm text-foreground h-[var(--size-control-sm)] w-[var(--space-16)] rounded-[var(--radius-md)] border px-[var(--space-2)]"
                inputMode="numeric"
                pattern="[0-9]*"
