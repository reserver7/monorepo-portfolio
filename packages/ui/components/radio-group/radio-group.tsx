...
function RadioGroupBase(
  ...
  const resolvedOrientation = resolveOption(
    ...
    RADIO_GROUP_DEFAULTS.orientation
  );
        ...
        return (
          <div key={option.value} className={cn("flex items-start gap-2.5", optionClassName)}>
            <RadioGroupItem
              ...
              className={cn(
                itemTopOffset,
                errorMessage ? "border-danger/50 focus-visible:ring-danger/20" : undefined
              )}
            />
            <Label
              htmlFor={optionId}
              size="sm"
              className={cn("cursor-pointer leading-5", optionLabelClassName)}
            >
    ...
    <RadioGroupPrimitive.Root
      ...
      className={cn(
        resolvedOrientation === "horizontal"
          ? "flex flex-wrap items-start gap-x-4 gap-y-2.5"
          : "grid gap-2.5",
        className
      )}
      value={value}
