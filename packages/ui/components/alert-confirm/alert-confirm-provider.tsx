...
import { ALERT_CONFIRM_DEFAULTS, UI_ALERT_CONFIRM_EVENT_NAME } from "./alert-confirm-provider.constants";
import {
  normalizeAlertOptions,
  normalizeConfirmOptions,
  normalizePromptConfirmOptions,
  normalizePromptValue,
  toFallbackText
} from "./alert-confirm-provider.utils";
import type {
...
export function AlertConfirmProvider() {
  ...
  const title =
    ...
    (current.kind === "alert" ? ALERT_CONFIRM_DEFAULTS.titleAlert : ALERT_CONFIRM_DEFAULTS.titleConfirm);
  const description = current.options.description;
  const confirmText = current.options.confirmText ?? ALERT_CONFIRM_DEFAULTS.confirmText;
  const confirmVariant = current.options.confirmVariant ?? ALERT_CONFIRM_DEFAULTS.confirmVariant;
  ...
  return (
    ...
    >
      <AlertConfirmContent>
        <AlertConfirmHeader>
          <AlertConfirmTitle>{title}</AlertConfirmTitle>
          {description ? <AlertConfirmDescription>{description}</AlertConfirmDescription> : null}
        </AlertConfirmHeader>
        ...
        {current.kind === "confirm" ? (
          <AlertConfirmFooter
            cancelText={current.options.cancelText ?? ALERT_CONFIRM_DEFAULTS.cancelText}
            confirmText={confirmText}
            cancelVariant={current.options.cancelVariant ?? ALERT_CONFIRM_DEFAULTS.cancelVariant}
        ...
        {current.kind === "prompt" ? (
          <AlertConfirmFooter>
            <Button
              variant={current.options.cancelVariant ?? ALERT_CONFIRM_DEFAULTS.cancelVariant}
              onClick={() => settleRequest(current, null)}
              disabled={promptSubmitting}
            >
              {current.options.cancelText ?? ALERT_CONFIRM_DEFAULTS.cancelText}
            </Button>
            <Button
