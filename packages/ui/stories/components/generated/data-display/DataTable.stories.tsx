...
const columns = [
  ...
  },
  {
    id: "severity",
    header: ({ column }: { column: { id: string } }) => (
      <DataTableColumnHeader column={column} title="심각도" />
    ),
    cell: ({ row }: { row: { original: IssueRow } }) => (
  ...
  },
  {
    ...
    cell: ({ row }: { row: { original: IssueRow } }) => (
      <Badge variant={statusToBadge[row.original.status]}>{row.original.status}</Badge>
    )
  },
  {
    id: "service",
  ...
  },
  {
    ...
    align: "right",
    header: ({ column }: { column: { id: string } }) => (
      <DataTableColumnHeader column={column} title="발생 횟수" />
    ),
    cellClassName: ({ value }: { value: unknown }) =>
      Number(value) >= 30 ? "text-danger font-semibold" : undefined
  },
  {
    id: "updatedAt",
    accessorKey: "updatedAt",
    header: ({ column }: { column: { id: string } }) => (
      <DataTableColumnHeader column={column} title="최근 발생" />
    )
  },
  {
    id: "action",
    ...
    return (
      <DataTable<IssueRow>
        ...
        filterFn={(row, activeFilters) => {
          const keyword = String(activeFilters.keyword ?? "")
            .trim()
            .toLowerCase();
        ...
        toolbar={({ query, setFilters, resetFilters }) => (
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              placeholder="키워드 검색"
              className="border-default bg-surface h-9 w-[220px] rounded-[var(--radius-md)] border px-3 text-sm"
            />
