  ...
  render: (args) => (
    <Tabs defaultValue="activity" className="w-full">
      <TabsList size={args.size} variant={args.variant} fullWidth={args.fullWidth}>
        <TabsTrigger value="activity" size={args.size} variant={args.variant}>
          활동
        </TabsTrigger>
        <TabsTrigger value="comments" size={args.size} variant={args.variant}>
          댓글
        </TabsTrigger>
        <TabsTrigger value="history" size={args.size} variant={args.variant}>
          이력
        </TabsTrigger>
      </TabsList>
    ...
    <section className="border-default bg-surface space-y-3 rounded-[var(--radius-xl)] border p-4">
      ...
      <div className="grid items-start gap-2 sm:grid-cols-2 xl:grid-cols-3">
        <div className="border-default bg-surface rounded-[var(--radius-md)] border p-3">
          ...
          <Tabs {...sanitizeStoryArgs(args as Record<string, unknown>)} />
        </div>
        <div className="border-default bg-surface rounded-[var(--radius-md)] border p-3">
          <div className="text-caption text-muted mb-2">전체 너비</div>
          <Tabs {...sanitizeStoryArgs(args as Record<string, unknown>)} fullWidth />
        </div>
      </div>
    </section>
  )
