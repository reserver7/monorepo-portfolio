...
export class OpsIssueService implements OnModuleInit {
  ...
  onModuleInit(): void {
    const timer = setInterval(
      () =>
        void this.escalateOverdueResponses().catch((error) =>
          this.logger.warn(
            `인시던트 자동 에스컬레이션 오류: ${error instanceof Error ? error.message : String(error)}`
          )
        ),
      60_000
    );
  ...
  async escalateOverdueResponses(): Promise<number> {
    ...
    const overdueAcknowledgements = await this.prisma.issue.findMany({
      where: {
        ...
        acknowledgedAt: null,
        escalationLevel: 0,
        firstOccurredAt: { lte: acknowledgementCutoff }
      },
      take: 50
    });
    const overdueUpdates = await this.prisma.issue.findMany({
      where: {
        ...
        escalationLevel: { lt: policy.maxLevel }
      },
      take: 50
    });
    let escalatedCount = 0;
    for (const [issue, reason] of [
      ...overdueAcknowledgements.map((issue) => [issue, "최초 확인 지연"] as const),
      ...overdueUpdates.map((issue) => [issue, "상태 공지 지연"] as const)
    ]) {
      ...
      const claim = await this.prisma.issue.updateMany({
        where: {
          ...
          ...(reason === "최초 확인 지연" ? { acknowledgedAt: null } : { nextUpdateAt: { lte: now } })
        },
        data: { escalationLevel, nextUpdateAt: null }
      });
      if (claim.count === 0) continue;
      escalatedCount += 1;
      const alert: OpsAlert = await this.prisma.opsAlert.create({
        data: {
          level: issue.severity === "critical" ? "critical" : "high",
          title: `${reason}: ${issue.title}`,
          message: `${reason}이 감지되었습니다. 지휘자 ${issue.commander || issue.assignee || "미지정"}에게 L${escalationLevel} 에스컬레이션이 필요합니다. 대상: ${policy.escalationTargets}`,
      ...
      await writeOpsAuditLog(this.prisma, this.logger, {
        action: "incident.auto_escalated",
        targetType: "Issue",
        targetId: issue.id,
        summary: `${issue.title} ${reason}(으)로 L${escalationLevel} 자동 에스컬레이션`,
        metadata: { escalationLevel, reason, policy }
      });
    }
  ...
  private async getEscalationPolicy(): Promise<{
    acknowledgeWithinMinutes: number;
    maxLevel: number;
    escalationTargets: string;
  }> {
    const setting = await this.prisma.opsSetting.findUnique({
      where: { key: "alert.escalation_policy" },
      select: { value: true }
    });
    const value = setting?.value as
      | { acknowledgeWithinMinutes?: unknown; maxLevel?: unknown; escalationTargets?: unknown }
      | undefined;
    const acknowledgeWithinMinutes =
      typeof value?.acknowledgeWithinMinutes === "number" && value.acknowledgeWithinMinutes > 0
        ? value.acknowledgeWithinMinutes
        : 10;
    const maxLevel = typeof value?.maxLevel === "number" ? Math.min(5, Math.max(1, value.maxLevel)) : 3;
    const escalationTargets =
      typeof value?.escalationTargets === "string" && value.escalationTargets.trim()
        ? value.escalationTargets.trim()
        : "Primary on-call → Backup on-call → Incident commander";
  ...
  async getIssueSummary(filter?: IssueFilterInput): Promise<IssueSummaryType> {
    ...
    const [open, criticalHigh, unassigned, slaRisk] = await this.prisma.$transaction([
      ...
      this.prisma.issue.count({
        where: { AND: [openWhere, { OR: [{ assignee: null }, { assignee: "" }] }] }
      }),
      this.prisma.issue.count({
        where: {
          AND: [
    ...
    const items: IncidentTimelineItemType[] = [
      {
        ...
        tone: issue.severity,
        occurredAt: issue.firstOccurredAt
      },
      ...(issue.deployment
        ? [
            {
              id: `deployment:${issue.deployment.id}`,
              kind: "deployment",
              title: `배포 ${issue.deployment.version}`,
              detail: issue.deployment.changelog,
              actor: issue.deployment.owner,
              tone: "warning",
              occurredAt: issue.deployment.deployedAt
            }
          ]
        : []),
      ...issue.logs.map((log) => ({
        id: `log:${log.id}`,
        kind: "log",
        title: `${log.level.toUpperCase()} 로그`,
        detail: log.rawMessage,
        tone: log.level.toLowerCase().includes("error") ? "critical" : "info",
  ...
  async updateIssueStatus(input: UpdateIssueStatusInput, actor?: string): Promise<IssueType> {
    ...
    const updated = await this.prisma.issue.update({
      ...
      data: {
        status,
        acknowledgedAt:
          !existing.acknowledgedAt && (status === "analyzing" || status === "in_progress") ? now : undefined,
    ...
    if (status === "resolved") {
      ...
      const snapshot =
        (await this.prisma.opsReportSnapshot.findFirst({
          where: { environment: updated.environment },
          orderBy: { generatedAt: "desc" }
        })) ??
        (await this.prisma.opsReportSnapshot.create({
          data: {
            title: `${updated.environment} 인시던트 후속 조치`,
            environment: updated.environment,
            riskLevel: "follow_up",
            executiveSummary: "해결된 인시던트의 재발 방지 조치를 추적합니다.",
            technicalSummary: "자동 생성된 운영 후속 조치 리포트입니다.",
            shareText: "OpsLens 인시던트 후속 조치"
          }
        }));
      await this.prisma.opsReportAction.upsert({
        where: {
          snapshotId_actionKey: { snapshotId: snapshot.id, actionKey: `incident-follow-up:${updated.id}` }
        },
        update: {},
        create: {
          snapshotId: snapshot.id,
          actionKey: `incident-follow-up:${updated.id}`,
          title: `${updated.title} 재발 방지 확인`,
          description: "Postmortem의 재발 방지 조치를 검증하고 완료 증빙을 남기세요.",
          owner: updated.assignee || actor || "운영 담당자",
          priority: updated.severity === "critical" ? "P0" : "P1",
  ...
  async updateIncidentClosure(input: UpdateIncidentClosureInput, actor?: string): Promise<IssueType> {
    ...
    await writeOpsAuditLog(this.prisma, this.logger, {
      actor,
      action: "issue.closure_updated",
      targetType: "Issue",
      targetId: updated.id,
      summary: `${updated.title} 종료 정보 업데이트`
    });
    return toIssueType({ ...updated, logs: [], comments: [] });
  }
  ...
  async updateIncidentResponse(input: UpdateIncidentResponseInput, actor?: string): Promise<IssueType> {
    ...
    await writeOpsAuditLog(this.prisma, this.logger, {
      ...
      metadata: {
        ...
        nextUpdateAt: updated.nextUpdateAt?.toISOString() ?? null
      }
    });
    return toIssueType({ ...updated, logs: [], comments: [] });
  }
  ...
  async bulkUpdateIssues(input: BulkUpdateIssuesInput, actor?: string): Promise<number> {
    ...
    if (issueIds.length === 0 || issueIds.length > 100)
      throw new BadRequestException("1~100개의 이슈를 선택하세요.");
    ...
    if (!status && input.assignee === undefined)
      throw new BadRequestException("변경할 상태 또는 담당자가 필요합니다.");
    const result = await this.prisma.issue.updateMany({
      where: { id: { in: issueIds } },
      data: {
        status,
        assignee: input.assignee?.trim() || undefined,
        acknowledgedAt: status === "analyzing" || status === "in_progress" ? new Date() : undefined,
    ...
    await writeOpsAuditLog(this.prisma, this.logger, {
      ...
      targetType: "Issue",
      summary: `${result.count}개 이슈 일괄 변경`,
      metadata: { issueIds, status, assignee: input.assignee }
    });
    return result.count;
  }
