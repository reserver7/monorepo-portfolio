...
export default function ServiceDetailPage() {
  ...
  const dependencyIssuesQuery = useQuery(useOpsQueryOptions("list", { queryKey: [...opslensQueryKeys.issues({ environment, serviceName: "all", search: "", status: "all", severity: "all", page: 1 }), "service-health"], queryFn: () => listIssues({ environment, page: 1, pageSize: 100 }) }));
  ...
  const dependencyHealth = dependencies.map((dependency) => {
    const relatedIssues = (dependencyIssuesQuery.data?.items ?? []).filter((item) => item.serviceName === dependency && item.status !== "resolved");
  ...
  if (hasCriticalDataError) {
    return (
      <FeedbackState
        variant="error"
        size="lg"
        title="서비스 운영 데이터를 불러오지 못했습니다."
        description="잠시 후 다시 시도하거나 API 상태를 확인해 주세요."
        action={
          <Button
            type="button"
            variant="secondary"
  ...
  return <OpsPageShell>
    ...
    <Grid className="gap-[var(--space-3)] md:grid-cols-2 xl:grid-cols-5"><MetricCard label="오픈 인시던트" value={formatNumber(openIssues.length)} helper={`Critical/High ${criticalHigh.length}건`} color={criticalHigh.length > 0 ? "danger" : "default"} /><MetricCard label="SLA 위험" value={formatNumber(slaRisk.length)} helper="기한 초과 이슈" color={slaRisk.length > 0 ? "warning" : "default"} /><MetricCard label="Error Budget" value={sloQuery.data?.budgetConsumed == null ? "—" : `${budgetRisk.toFixed(1)}%`} helper={sloQuery.data?.availability == null ? "메트릭 수신 대기" : `가용성 ${sloQuery.data.availability.toFixed(3)}% · 목표 ${sloQuery.data.target}%`} color={budgetRisk >= 70 ? "danger" : budgetRisk >= 35 ? "warning" : "default"} /><MetricCard label="수집 상태" value={`${staleSources.length}개`} helper="30분 이상 수신 지연" color={staleSources.length > 0 ? "warning" : "default"} /><MetricCard label="온콜" value={service?.onCall || "미지정"} helper={service?.owner ? `오너 ${service.owner}` : "카탈로그에서 지정"} /></Grid>
    <Grid className="gap-[var(--space-4)] xl:grid-cols-3"><OpsSectionCard title="활성 인시던트" description="심각도와 최근 발생 시각 기준입니다." className="xl:col-span-2">{openIssues.length ? <Box className="space-y-[var(--space-2)]">{openIssues.slice(0, 8).map((issue) => <Link key={issue.id} href={`/issues/${issue.id}`} className="border-default hover:border-primary/40 block rounded-[var(--radius-md)] border p-[var(--space-3)]"><Flex className="items-start justify-between gap-[var(--space-2)]"><Box className="min-w-0"><Typography as="p" variant="bodySm" className="font-semibold">{issue.title}</Typography><Typography as="p" variant="caption" color="muted" className="mt-[var(--space-1)]">담당 {issue.assignee || "미지정"} · 최근 {formatDateTime(issue.lastOccurredAt)}</Typography></Box><SeverityBadge severity={issue.severity} /></Flex></Link>)}</Box> : <FeedbackState variant="empty" size="sm" title="열린 인시던트가 없습니다." />}</OpsSectionCard><OpsSectionCard title="연결 리소스" description="운영 대응에 필요한 서비스 연결입니다."><Flex className="flex-wrap gap-[var(--space-2)]">{service?.repository ? <Button asChild variant="outline" size="sm"><Link href={service.repository} target="_blank">저장소</Link></Button> : null}{service?.dashboard ? <Button asChild variant="outline" size="sm"><Link href={service.dashboard} target="_blank">대시보드</Link></Button> : null}</Flex><Box className="mt-[var(--space-3)]"><Typography as="p" variant="caption" color="muted">의존 서비스 건강도</Typography><Box className="mt-[var(--space-1)] space-y-[var(--space-1)]">{dependencyHealth.length ? dependencyHealth.map((dependency) => <Flex key={dependency.name} className="items-center justify-between gap-[var(--space-2)]"><Button asChild variant="ghost" size="sm"><Link href={`/services/${encodeURIComponent(dependency.name)}`}>{dependency.name}</Link></Button><Badge size="sm" variant={dependency.urgent ? "danger" : dependency.openCount > 0 ? "warning" : "success"}>{dependency.urgent ? "영향 확인" : dependency.openCount > 0 ? `열린 이슈 ${dependency.openCount}` : "정상"}</Badge></Flex>) : <Typography as="p" variant="caption" color="muted">등록되지 않음</Typography>}</Box></Box><Box className="mt-[var(--space-3)]"><Typography as="p" variant="caption" color="muted">이 서비스에 의존</Typography><Flex className="mt-[var(--space-1)] flex-wrap gap-[var(--space-1)]">{dependents.length ? dependents.map((dependent) => <Button key={dependent.name} asChild variant="outline" size="sm"><Link href={`/services/${encodeURIComponent(dependent.name ?? "")}`}>{dependent.name}</Link></Button>) : <Typography as="p" variant="caption" color="muted">등록되지 않음</Typography>}</Flex></Box></OpsSectionCard><OpsSectionCard title="최근 배포" description="서비스 범위 태그와 연결된 최신 배포입니다." className="xl:col-span-3">{latestDeployment ? <Box className="border-default rounded-[var(--radius-md)] border p-[var(--space-3)]"><Typography as="p" variant="bodySm" className="font-semibold">{latestDeployment.version}</Typography><Typography as="p" variant="caption" color="muted" className="mt-[var(--space-1)]">{formatDateTime(latestDeployment.deployedAt)} · 승인 {latestDeployment.approvalStatus}</Typography><Typography as="p" variant="caption" color="muted" className="mt-[var(--space-2)]">{latestDeployment.changelog}</Typography><Button asChild variant="outline" size="sm" className="mt-[var(--space-3)]"><Link href="/deployments">배포 상세 보기</Link></Button></Box> : <FeedbackState variant="empty" size="sm" title="연결된 배포 이력이 없습니다." />}</OpsSectionCard></Grid>
  </OpsPageShell>;
}
