...
export default function ServiceDetailPage() {
  ...
  return (
    <OpsPageShell>
      <OpsSectionCard
        title={`${serviceName} 서비스 운영`}
        description={`${environment} 환경 기준의 인시던트, 배포, SLO 운영 현황입니다.`}
      >
        <Flex className="flex-wrap items-center gap-[var(--space-2)]">
          ...
          >
            {criticalHigh.length > 0 ? "인시던트" : openIssues.length > 0 ? "주의" : "정상"}
          </Badge>
          ...
          <Button asChild variant="ghost" size="sm">
            <Link href={`/issues?service=${encodeURIComponent(serviceName)}`}>이슈 전체 보기</Link>
          </Button>
        </Flex>
      </OpsSectionCard>
      <Grid className="gap-[var(--space-3)] md:grid-cols-2 xl:grid-cols-5">
        <MetricCard
          ...
          helper={`Critical/High ${criticalHigh.length}건`}
          color={criticalHigh.length > 0 ? "danger" : "default"}
        />
        <MetricCard
          label="SLA 위험"
          value={formatNumber(slaRisk.length)}
          helper="기한 초과 이슈"
        ...
        <MetricCard
          ...
          helper={
            sloQuery.data?.availability == null
              ? "메트릭 수신 대기"
              : `가용성 ${sloQuery.data.availability.toFixed(3)}% · 목표 ${sloQuery.data.target}%`
          }
          color={budgetRisk >= 70 ? "danger" : budgetRisk >= 35 ? "warning" : "default"}
        />
        <MetricCard
          label="수집 상태"
          value={`${staleSources.length}개`}
          helper="30분 이상 수신 지연"
          color={staleSources.length > 0 ? "warning" : "default"}
        />
        <MetricCard
          label="온콜"
          value={service?.onCall || "미지정"}
          helper={service?.owner ? `오너 ${service.owner}` : "카탈로그에서 지정"}
        />
      </Grid>
      <Grid className="gap-[var(--space-4)] xl:grid-cols-3">
        <OpsSectionCard
          title="활성 인시던트"
          description="심각도와 최근 발생 시각 기준입니다."
        ...
        >
          {openIssues.length ? (
                  ...
                  <Flex className="items-start justify-between gap-[var(--space-2)]">
                    <Box className="min-w-0">
                      ...
                      <Typography as="p" variant="caption" color="muted" className="mt-[var(--space-1)]">
                        담당 {issue.assignee || "미지정"} · 최근 {formatDateTime(issue.lastOccurredAt)}
                      </Typography>
                    </Box>
            ...
            <FeedbackState variant="empty" size="sm" title="열린 인시던트가 없습니다." />
          )}
        </OpsSectionCard>
        <OpsSectionCard title="연결 리소스" description="운영 대응에 필요한 서비스 연결입니다.">
        ...
        <OpsSectionCard
          title="최근 배포"
          description="서비스 범위 태그와 연결된 최신 배포입니다."
        ...
        >
          {latestDeployment ? (
            <Box className="border-default rounded-[var(--radius-md)] border p-[var(--space-3)]">
              ...
              <Typography as="p" variant="caption" color="muted" className="mt-[var(--space-1)]">
                {formatDateTime(latestDeployment.deployedAt)} · 승인 {latestDeployment.approvalStatus}
              </Typography>
            ...
            <FeedbackState variant="empty" size="sm" title="연결된 배포 이력이 없습니다." />
          )}
        </OpsSectionCard>
      </Grid>
    </OpsPageShell>
