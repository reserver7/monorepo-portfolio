...
export function AlertsModal({
  ...
  useEffect(() => {
    try {
      setSnoozedUntil(
        JSON.parse(window.localStorage.getItem("opslens-alert-snoozes") ?? "{}") as Record<string, number>
      );
    } catch {
      setSnoozedUntil({});
    }
  ...
  const sourceOptions = useMemo(
    () => [
      { label: "모든 소스", value: "all" },
      ...Array.from(new Set(alerts.map((alert) => alert.source).filter(Boolean))).map((source) => ({
        label: source,
        value: source
      }))
    ],
      ...
      Array.from(
        alerts
          .reduce((groups, alert) => {
            ...
            return groups;
          }, new Map<string, { key: string; source: string; title: string; count: number; unread: number }>())
          .values()
      )
  ...
  const recentAlerts = useMemo(
    () =>
      alerts
        ...
        .slice(0, 30),
    [alerts, levelFilter, readFilter, snoozedUntil, sourceFilter]
  );
  const levelLabelMap = {
    critical: "critical",
    high: "high",
          ...
          <Flex className="border-default bg-surface-elevated items-center justify-between rounded-[var(--radius-md)] border px-[var(--space-3)] py-[var(--space-2)]">
            <Flex className="items-center gap-[var(--space-2-5)]">
              ...
              <Badge
                ...
                size="sm"
                shape="rounded"
                className="border-current/20 border font-semibold"
              >
                {unreadCount}
              </Badge>
            </Flex>
            {unreadCount > 0 ? (
              <Button
                ...
                className="text-caption text-muted hover:!bg-surface hover:text-foreground h-7 border-transparent !bg-transparent px-[var(--space-2)]"
            ...
            <Box className="border-default bg-surface-elevated rounded-[var(--radius-md)] border p-[var(--space-2)]">
              ...
              <Box className="mt-[var(--space-1)] space-y-[var(--space-1)]">
                {groupedSummary.map((group) => (
                  <Flex key={group.key} className="items-center justify-between gap-[var(--space-2)]">
                  ...
                  return (
                    ...
                    >
                      <Flex className="items-start gap-[var(--space-2)]">
                        <Box
                          className={`mt-[var(--space-1-5)] h-[var(--space-1-5)] w-[var(--space-1-5)] shrink-0 rounded-full ${
                        ...
                        <Box className="min-w-0 flex-1">
                          <Typography
                            as="p"
                            variant="caption"
                            className={`line-clamp-2 pr-[var(--space-2)] ${unread ? "text-foreground font-semibold" : "text-muted"}`}
                          >
                            {alert.title}
                          </Typography>
                          <Flex className="mt-[var(--space-1)] flex-wrap items-center gap-x-[var(--space-2)] gap-y-[var(--space-1)]">
                            <Typography
                              as="span"
                              variant="caption"
                              className={`font-semibold uppercase ${levelTextClassMap[alert.level]}`}
                            >
                      ...
                      <Button
                        ...
                        className="text-muted hover:!bg-danger/10 hover:text-danger absolute right-[var(--space-1)] top-[var(--space-2)] h-6 w-6 border-transparent !bg-transparent opacity-0 focus-visible:opacity-100 group-hover:opacity-70"
                        onClick={(event) => {
                          event.stopPropagation();
